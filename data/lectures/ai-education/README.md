...
마스터 v1.12와 사용자가 전달한 v3를 기준으로 본문 문체, 예시 표기와 하단 원본 이미지를 갱신했다. 새로 추가된 이미지 개체는 40개이며 기존 20개를 포함해 총 60개다. 문장과 출처는 v3의 원문을 그대로 사용했다. 기존 발표자 노트에 v3에서 추가한 그림 발문도 별도 참고 창에 반영했다. 원본 PPTX와 마스터는 수정하지 않았다.
