...
(async()=>{
 ...
 function resize(){if(!data||data.slides[index].kind==='lab')return;const scale=stage.clientWidth/data.width*(zoom?2:1);canvas.style.transform=`scale(${scale})`;$('#ae-size').style.width=`${data.width*scale}px`;$('#ae-size').style.height=`${data.height*scale}px`}
 ...
 function render(i,write=true){
  ...
  const isLab=s.kind==='lab';stage.classList.toggle('lab-mode',isLab);$('#ae-size').hidden=isLab;$('#ae-lab').hidden=!isLab;$('#ce-zoom').hidden=isLab;
  if(isLab){window.aiEducationLabs.mount($('#ae-lab'),s);$('#ae-lab').scrollTo(0,0)}else{
  canvas.innerHTML=`<img class="ae-background" src="${esc(s.background)}" alt="" draggable="false">`+s.pictures.map(p=>{const c=p.crop,dx=1-c.l-c.r,dy=1-c.t-c.b;return `<div class="ae-picture" style="left:${p.x}px;top:${p.y}px;width:${p.w}px;height:${p.h}px"><img src="${esc(p.src)}" alt="${esc(s.title)} 원본 그림" draggable="false" style="${p.zoomOnly?'visibility:hidden;':''}left:${-c.l/dx*100}%;top:${-c.t/dy*100}%;width:${100/dx}%;height:${100/dy}%"></div>`}).join('')+s.runs.map(r=>`<span class="ae-run" aria-hidden="true" data-width="${r.w}" style="left:${r.x}px;top:${r.y}px;font-size:${r.size}px;font-weight:${weight(r)};font-style:${r.italic?'italic':'normal'};color:${r.color};line-height:${r.h}px">${esc(r.text)}</span>`).join('');
  if(s.context){const note=document.createElement('p');note.className='ch4-source-context';note.textContent=s.context;canvas.append(note)}
  ...
  if(data.slides[index+1]?.background){const im=new Image();im.src=data.slides[index+1].background}
 }
 function hashIndex(){const m=location.hash.match(/^#slide-(\d+)$/);return m?Number(m[1])-1:0}
 try{
  const res=await fetch(`/data/lectures/ai-education/${viewer.dataset.chapter}.json?v=20260915sources`);if(!res.ok)throw Error('자료를 불러오지 못했습니다.');data=await res.json();
  ...
  $('#ce-open-links').onclick=()=>$('#ce-links-dialog').showModal();
