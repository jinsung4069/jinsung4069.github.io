...
(async()=>{
 ...
 function render(i,write=true){
  index=Math.max(0,Math.min(data.slides.length-1,i));const s=data.slides[index],isLab=s.kind==='lab';
 ...
 try{
  const response=await fetch(`../data/lectures/computer-education2/${viewer.dataset.chapter}.json?v=glass2`);if(!response.ok)throw Error('자료를 불러오지 못했습니다.');data=await response.json();page.max=data.slides.length;
